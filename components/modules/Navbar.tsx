...
export default function Navbar() {
    ...
    const navigation = [
        { title: "ورود به سایت دابزهوم", path: "#" },
        { title: "مزایای ثبت خانه در دابزهوم", path: "#" },
  ...
  return (
    <>
    <div onClick={()=> {state && setState(false)}} className={` bg-[#26397F] lg:pt-5 ${state && 'w-full fixed bg-[#26397f8e]'}`}>
    <nav  className={`${state && 'w-[80%] mx-0  bg-[rgb(255,255,255,100%)]'} bg-[rgb(255,255,255,50%)]  lg:w-[85%] mx-auto lg:rounded-[12px] font-bold text-[1.1rem]`}>
          ...
          <div className="items-center px-4 md:px-8 lg:flex justify-between">
                ...
                <div className='flex items-center gap-x-4'>
                  ...
                  <div className="lg:hidden">
                      ...
                      >
                          {
                              state ? (
                                  <svg  style={{color: '#26397F'}} xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className={`lg:overflow-visible lg:flex lg:pb-0 lg:pr-0 lg:h-auto ${ state ? 'h-screen pt-5 overflow-auto pr-2' : 'hidden'} `}>
                    <div className="flex">
                        <ul className="items-center space-y-8 lg:flex lg:space-x-5 lg:space-y-0 w-fit">
                            {
                               navigation.map((item, idx) => (
                                <li key={idx}>
                                    {item.title !== 'ورود به سایت دابزهوم' && (
                                        <Link href={item.path} className='lg:mr-4 lg:text-white text-[#26397F]'>
                                            {item.title}
                                        </Link>
                                    )}
                            
                                    {item.title === 'ورود به سایت دابزهوم' && state && (
                                        <Link href={item.path} className='lg:mr-4 lg:text-white text-[#26397F]'>
                                            {item.title}
                                        </Link>
                                    )}
